...
void Displays::setDisplayPaging(bool paging)
{
	...
	displayPaging = paging;
}

void Displays::stopPageDisplays()
{
...
void Displays::printPersistantInfo(U8G2_FOR_ADAFRUIT_GFX &gfx, int &sid, const char * &m) {
	...
	gfx.setFont(u8g2_font_crox1hb_tr);

	gfx.setCursor(disp_w - 20, padding_screen_top);
	gfx.print(sid);

	gfx.setCursor(padding_screen_left, padding_screen_top);
	gfx.print(m);
...
/****************************************/
/*!
	@brief Display Id, message, invert or not
	@param d 	Id of display to print to
	@param msg	The message
	@param inv	Invert the display before printing the message 
*/
/*****************************************/
void Displays::printMessage(int& d, const char*& msg, bool inv)
{
	displayArray[d].clearDisplay();
	u8g2_for_adafruit_gfx.begin(displayArray[d]);
	u8g2_for_adafruit_gfx.setFont(u8g2_font_crox2hb_tr);
	displayArray[d].stopscroll();
	

	// const char *updateMsg = "UPDATED";
	int16_t text_width = u8g2_for_adafruit_gfx.getUTF8Width(msg);

	int16_t text_center_x = disp_center_x - (text_width / 2);
	int16_t text_center_y = disp_center_y + (character_height / 2);
...
void Displays::loop()
{
	if (displayPaging)
	{
		// Serial.println("paging displays");
		...
		if (currentMillis - previousMillis >= pageInterval)
		{
			...
			else if (screenMode == "spool_weight")
			{
				screenMode = "overview";
				spoolWeightDisplay();
			}
